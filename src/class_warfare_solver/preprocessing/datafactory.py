...
class DataFactory():
    ...
    def _build_character_features(self):
        ...
        for col_name in character_df.columns:
            if col_name not in ["Game", "Class", "Name", "Mov"] and "class" not in col_name:
                character_df = character_df.withColumn(col_name, F.when(F.col("Game") != 13, F.cast(int, (F.col(col_name) - F.col("class_"+col_name)))).otherwise(F.col(col_name)))
        character_df = character_df.withColumn("Mov", F.cast(int, F.when(F.col("Game").isin([4,13]), 0).otherwise(F.col("Mov") - F.col("class_Mov"))))

        character_df = character_df.select(["Game", "Name", "Class", "HP", "Str", "Mag", "Skl", "Spd", "Lck", "Def", "Res", "Mov", "gHP", "gStr", "gMag", "gSkl", "gSpd", "gLck", "gDef", "gRes"])
    ...
    def _clean_character_growths(self):
        ...
        for col in ["HP", "Str", "Mag", "Skl", "Spd", "Lck", "Def", "Res"]:
            character_growths = character_growths.withColumn("g"+col, F.when(F.col(col) == "–", 0).otherwise(F.col(col)).cast("int"))

        self._character_growths = character_growths.select([
            "Game", "Name",  "gHP", "gStr", "gMag", "gSkl", "gSpd", "gLck", "gDef", "gRes",
        ])
        return None
